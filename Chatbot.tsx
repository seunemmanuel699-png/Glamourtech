...
const WEBHOOK_URL =
  "https://hook.us2.make.com/fvvlanaosnlakmzbrdbya9mazf1w9oyo";
...
const Chatbot: React.FC = () => {
  ...
  const handleSend = async (overrideInput?: string) => {
    ...
    try {
      ...
      const cleanResponse = (
        modelText ||
        "I've processed your request but the response was empty. Please check your automation configuration."
      ).replace(/\*\*/g, "");

      setMessages((prev) => [...prev, { role: "model", text: cleanResponse }]);
      triggerNotification(
        "Automation Consultant",
        cleanResponse.length > 70
          ? cleanResponse.substring(0, 70) + "..."
          : cleanResponse,
        "chat",
      );
    } catch (error) {
      ...
      const errText =
        "I'm having trouble reaching our automation hub. Please ensure the Webhook Response is active in your workflow.";
      setMessages((prev) => [...prev, { role: "model", text: errText }]);
      triggerNotification(
        "Hub Connectivity Issue",
        "Failed to synchronize conversation with the central webhook.",
        "chat",
      );
    } finally {
      setIsLoading(false);
      ...
      {isOpen && (
        <div className="absolute bottom-20 right-0 w-[360px] md:w-[420px] h-[600px] max-h-[80vh] bg-brand-black border border-white/10 rounded-sm shadow-2xl flex flex-col overflow-hidden animate-in fade-in slide-in-from-bottom-6 duration-300">
          <div className="bg-[#0066FF] p-5 flex items-center justify-between shadow-lg">
            <div>
              <h3 className="text-white font-bold heading-font text-sm uppercase tracking-widest">
                Automation Hub
              </h3>
              <p className="text-white/80 text-[10px] uppercase font-bold tracking-tighter">
                Powered by Make.com
              </p>
            </div>
            <button
          ...
          <div className="flex-grow overflow-y-auto p-6 space-y-6 bg-brand-black">
            {messages.map((m, i) => (
              ...
              >
                <div
                  className={`max-w-[90%] p-4 text-sm leading-relaxed rounded-sm ${
                    m.role === "user"
