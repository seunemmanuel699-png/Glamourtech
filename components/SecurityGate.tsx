...
export const SecurityGate: React.FC<SecurityGateProps> = ({ onAccessGranted }) => {
  ...
  return (
    <motion.div 
      ...
      className="fixed inset-0 z-[99999] bg-brand-black/95 backdrop-blur-2xl flex items-center justify-center p-4 selection:bg-brand-red selection:text-white"
    >
      {/* Background ambient lighting */}
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[500px] h-[500px] bg-brand-red/10 rounded-full blur-[140px] pointer-events-none" />
      ...
      >
        {/* Top Accent Line */}
        <div className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-brand-red via-[#00D2FF] to-brand-red" />
          ...
          <div className="relative mb-4">
            <div className={`w-16 h-16 rounded-full flex items-center justify-center transition-all duration-500 border ${
              isVerified 
                ...
                : 'bg-brand-red/10 border-brand-red/30 text-brand-red shadow-[0_0_30px_rgba(255,0,51,0.2)]'
            }`}>
              {isVerified ? (
            ...
            <span className="absolute -bottom-1 -right-1 flex h-4 w-4">
              <span className={`animate-ping absolute inline-flex h-full w-full rounded-full opacity-75 ${
                isVerified ? 'bg-green-400' : 'bg-brand-red'
              }`} />
              <span className={`relative inline-flex rounded-full h-4 w-4 ${
                isVerified ? 'bg-green-500' : 'bg-brand-red'
              }`} />
            </span>
          </div>

          <span className="text-[9px] font-black uppercase tracking-[0.4em] text-brand-red mb-1">
            Glamourtech AI Guard
          </span>
          <h2 className="text-2xl font-black heading-font uppercase text-white tracking-tight">
            Security <span className="text-brand-red">Verification</span>
          </h2>
