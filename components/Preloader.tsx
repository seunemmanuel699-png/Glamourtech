  ...
  return (
    <div className={`fixed inset-0 z-[9999] bg-brand-black flex flex-col items-center justify-center transition-opacity duration-1000 ${isReady ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}>
      ...
      <div className="w-48 h-[2px] bg-white/10 relative overflow-hidden rounded-full">
        <div className={`absolute inset-y-0 left-0 bg-gradient-to-r from-[#0066FF] to-[#00D2FF] transition-all duration-[3000ms] ease-out ${isReady ? 'w-full' : 'w-1/3 animate-pulse'}`}></div>
      </div>
      
      <div className="mt-6 flex flex-col items-center space-y-2">
        <span className="text-[#00D2FF] font-black text-[9px] uppercase tracking-[0.5em] animate-pulse">Initializing Core Systems</span>
